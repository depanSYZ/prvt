...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const newSession = await createSession({
      username: newData.username, email: newData.email,
      avatar: newData.avatar, apikey: newData.apikey, plan: newData.plan,
    });

    const res = NextResponse.json({ author: "Snaptok", success: true, newUsername: newData.username });
    res.cookies.set(COOKIE_NAME, newSession, COOKIE_OPTIONS);
    return res;
  } catch {
