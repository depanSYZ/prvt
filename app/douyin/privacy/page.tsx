"use client";
// app/douyin/privacy/page.tsx
// Uses the same unified Privacy & Terms content as /privacy
// Just swaps Header/Footer for Douyin-branded variants
...
export default function DouyinPrivacyPage() {
  const { lang } = useLang();
  return (
    <div className="flex min-h-screen flex-col">
      <DouyinHeader />
      <main className="flex-1">
        {/* Hero — red accent for Douyin */}
        <div className="relative overflow-hidden py-16 sm:py-20 px-4 border-b border-border">
          ...
          <div className="container mx-auto max-w-4xl text-center relative">
            <div className="inline-flex items-center justify-center rounded-2xl bg-red-500/10 border border-red-500/20 p-4 mb-6 shadow-lg">
              <Shield className="h-9 w-9 text-red-500" />
            </div>
            ...
            <div className="mt-6 flex flex-wrap items-center justify-center gap-3">
              ...
              <div className="inline-flex items-center gap-2 rounded-full border border-red-500/20 bg-red-500/5 px-4 py-1.5 text-xs text-red-500">
        ...
        {/* Sections — identical content to /privacy, same i18n source */}
        <div className="container mx-auto max-w-4xl px-4 py-12 space-y-4">
          ...
          <div className="flex flex-wrap gap-3 justify-center pt-2">
            <Link href="/douyin/guide" className="text-xs text-red-500/70 hover:text-red-500 hover:underline flex items-center gap-1">
              <ChevronRight className="h-3 w-3" />{t(tr.nav.guide, lang)}
            </Link>
            <Link href="/douyin/faq" className="text-xs text-red-500/70 hover:text-red-500 hover:underline flex items-center gap-1">
              <ChevronRight className="h-3 w-3" />{t(tr.nav.faq, lang)}
            </Link>
            <Link href="/privacy" className="text-xs text-red-500/70 hover:text-red-500 hover:underline flex items-center gap-1">
              <ChevronRight className="h-3 w-3" />
              {lang === "en" ? "TikTok Privacy" : lang === "id" ? "Privasi TikTok" : "TikTok Privacy"}
            </Link>
          </div>
        </div>
      </main>
