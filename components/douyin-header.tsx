...
import { PlayCircle, Home, HelpCircle, Shield, BookOpen, FileText, User, LogIn, X } from "lucide-react";
...
export function DouyinHeader() {
  ...
  const navLinks = [
    { href: "/douyin",         label: t(tr.nav.home,    lang), icon: Home },
    { href: "/douyin/faq",     label: t(tr.nav.faq,     lang), icon: HelpCircle },
    { href: "/douyin/privacy", label: t(tr.nav.privacy, lang), icon: Shield },
    { href: "/douyin/guide",   label: t(tr.nav.guide,   lang), icon: BookOpen },
    ...
    <>
      <header className={`sticky top-0 z-50 w-full border-b border-border transition-all duration-300 ${scrolled ? "bg-background/98 shadow-sm backdrop-blur-md" : "bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60"}`}>
        <div className="container mx-auto flex h-16 items-center justify-between px-4">
          <Link href="/douyin" className="flex items-center gap-2 text-xl font-bold text-foreground transition-colors hover:text-foreground/80">
          ...
          <div className="hidden md:flex items-center gap-1 rounded-full border border-border bg-muted p-1 text-sm">
            <Link href="/" className="rounded-full px-4 py-1.5 font-medium text-muted-foreground transition-colors hover:bg-background hover:text-foreground">🎵 TikTok</Link>
            <span className="rounded-full px-4 py-1.5 font-medium bg-background text-foreground shadow-sm">🎬 Douyin</span>
          </div>
              ...
              {loggedIn ? (
                ...
                <div className="flex items-center gap-1">
                  ...
                  <Link href="/auth/register">
                    <Button size="sm" className="h-8 text-xs px-3 bg-red-500 hover:bg-red-600">{lReg}</Button>
                  </Link>
                </div>
              )}
            </div>
          ...
          <div className="flex items-center gap-2 md:hidden">
            ...
            <button
              ...
              className={`relative w-10 h-10 flex items-center justify-center rounded-xl transition-all duration-200 ${isOpen ? "bg-red-500/10 text-red-500" : "hover:bg-muted text-foreground"}`}
            >
      ...
      {/* Drawer */}
      <div className={`fixed top-0 right-0 z-50 md:hidden h-full w-[300px] sm:w-[340px] bg-background shadow-2xl transform transition-transform duration-300 ease-in-out flex flex-col ${isOpen ? "translate-x-0" : "translate-x-full"}`}>
        {/* Drawer header */}
        <div className="flex items-center justify-between px-5 py-4 border-b border-border">
          <Link href="/douyin" className="flex items-center gap-2 text-lg font-bold" onClick={() => setIsOpen(false)}>
        ...
        <div className="px-5 py-4 border-b border-border">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link href="/douyin" onClick={() => setIsOpen(false)}
              className="flex-1 flex items-center justify-center gap-2 rounded-xl border border-red-200 bg-red-50 dark:bg-red-950/20 dark:border-red-900/30 px-3 py-2.5 text-sm font-medium text-red-600 dark:text-red-400">
              🎬 Douyin
            </Link>
          </div>
        </div>
          ...
          {loggedIn ? (
            ...
            <>
              ...
              <Link href="/auth/register" onClick={() => setIsOpen(false)}>
                <Button className="w-full rounded-xl h-11 bg-red-500 hover:bg-red-600">{lReg}</Button>
              </Link>
            </>
          )}
        </div>
